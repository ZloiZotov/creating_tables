...
app.controller('tableCntr', function($scope){
  ...
  $scope.initSample = function() {
    var table = new Table("Пример таблицы", ['name','value']);
    table.data.push([1, 'One', 'h5jc54hdym'],
                    [2, 'Two','54j5kc6k'],
                    [3, 'Three', 'r0xgu59g4']);
  ...
  $scope.initSample();
   ...
   $scope.createTable = function() {
    var name = prompt('Название таблицы');
    var schema = prompt('Укажите названия колонок через запятую').split(",");
   ...
   if (confirm("Удалить таблицу? Уверены?")) $scope.TABLES.splice(i,1);
 };
...
var Table = function(name, schema) {
  ...
  this.insert = function() {
    var inp = prompt('Перечислите значения в строке через запятую').split(",");
  ...
  this.modifyData = function(x,y) {
    var change = prompt("Новое название для выбранной ячейки").trim();
